...
class PSICalculator:
    ...
    def evaluate_psi10(self, row: pd.Series, appendix: Dict[str, Set[str]]) -> Tuple[str, str]:
        ...
        has_aki_dx = False
        for dx_entry in all_diagnoses[1:]: # Iterate through secondary diagnoses
            if dx_entry['code'] in appendix.get('PHYSIDB', set()) and (dx_entry['poa'] in ['N', 'U', 'W', None] or pd.isna(poa_status)):
                has_aki_dx = True
                break
    ...
    def evaluate_psi15(self, row: pd.Series, appendix: Dict[str, Set[str]]) -> Tuple[str, str]:
        ...
        for injury_dx_entry in all_diagnoses[1:]: # Start from DX1 onwards
            ...
            if injury_poa_status == 'Y':
                ...
                # Only exclude if ANY procedure from the matching_proc_code_set_name exists
            ...
            # Check if diagnosis is NOT POA (N, U, W, None, or NaN)
    ...
    def evaluate_psi17(self, row: pd.Series, appendix: Dict[str, Set[str]]) -> Tuple[str, str]:
        ...
        principal_dx_code = all_diagnoses[0]['code'] if all_diagnoses else None
        if not (principal_dx_code and principal_dx_code.strip().upper() in set(code.strip().upper() for code in appendix.get('NEWBORN', set()))):
...
if __name__ == "__main__":
    ...
    # PSI-05 Logic: Retained Surgical Item or Unretrieved Device Fragment
        # PSI-05 Logic: Retained Surgical Item or Unretrieved Device Fragment
    # PSI-05 Logic: Retained Surgical Item or Unretrieved Device Fragment
